...
def parse_fve(distinguishedName, bitlocker_keys):
    ...
    matched = re.match(
        r"^(CN=)([0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9]-[0-9][0-9]:[0-9][0-9])({[0-9A-F\-]+}),",
        distinguishedName,
        re.IGNORECASE,
    )
...
def parseArgs():
    print("ExtractBitlockerKeys.py v%s \n" % VERSION)
...
if __name__ == "__main__":
    ...
    if len(computer_keys.keys()) != 0:
        ...
        print(
            "[!] No computers in the domain found matching filter '%s'"
            % options.ldap_query
        )
